...
const displayTickets = (companyData) => {
	...
	for (let i = 0; i < companyData.data.results.length; i++) {
		...
		appendData += `<div class="row"><div class="flx">
        <div class="col-sm-4 inner"></div>
        <div class="col-sm-8 inner-end">
        <h2>Ticket Information ${i} </h2>
        <p>Subject : ${companyData.data.results[i].subject}</p>
        <p>Ticket ID : ${companyData.data.results[i].id}</p>
        <p>Requester ID : ${companyData.data.results[i].requester_id}</p>
        <p>Type : ${status_check}</p>
        <p>Type : ${companyData.data.results[i].type}</p>
        </div>
        </div>
      </div>`
	}
