...
const displayTickets = (companyData, apikey, subdomain) => {
	...
	for (let i = 0; i < companyData.data.results.length; i++) {
		...
		getAgent(companyData.data.results[i].requester_id, apikey, subdomain)
			.then((res) => {
				console.log("res::", res)
                ...
                <p><b>Ticket ID </b>: ${companyData.data.results[i].id}&nbsp &nbsp &nbsp<b>Name </b>: ${res}&nbsp &nbsp &nbsp<b>Status</b> : ${status_check}&nbsp &nbsp &nbsp<b>Type </b>: ${companyData.data.results[i].type}</p>
                </div>
                </div>
              </div>`
			...
			.catch((err) => {
				getContact(
					companyData.data.results[i].requester_id,
					apikey,
					subdomain
				)
					.then((res) => {
...
function getContact(requester_id, apikey, subdomain) {
	return new Promise(function (resolve, reject) {
		...
		axios.get(url, options).then(
			function (companyData) {
				console.log("company Contacts", companyData)
				resolve(companyData.data.contact.name)
			},
			(error) => {
				reject("Error")
