...
const displayTickets = (companyData, apikey, subdomain) => {
	...
	for (let i = 0; i < companyData.data.results.length; i++) {
		...
		getContact(companyData.data.results[i].requester_id)
        ...
        <p><b>Ticket ID </b>: ${companyData.data.results[i].id}&nbsp &nbsp &nbsp<b>Name </b>: ${companyData.data.results[i].requester_id}&nbsp &nbsp &nbsp<b>Status</b> : ${status_check}&nbsp &nbsp &nbsp<b>Type </b>: ${companyData.data.results[i].type}</p>
        </div>
        </div>
      </div>`
	}
...
function getContact(apikey, subdomain) {
