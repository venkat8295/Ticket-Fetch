...
const displayTickets = (companyData) => {
	...
	for (let i = 0; i < companyData.data.results.length; i++) {
        ...
        <p><b>Ticket ID </b>: ${companyData.data.results[i].id}&nbsp &nbsp<b>Requester ID </b>: ${companyData.data.results[i].requester_id}&nbsp<b>Status</b> : ${status_check}&nbsp<b>Type </b>: ${companyData.data.results[i].type}</p>
        </div>
        </div>
      </div>`
	}
