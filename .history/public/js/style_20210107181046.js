...
const getContact = (requester_id, apikey, subdomain, options) => {
	return new Promise(function (resolve, reject) {
		const url2 = `https://${subdomain}.freshdesk.com/api/v2/contacts/26010904307`

		axios
			.get(url2, options2)
			.then((companyData2) => {
				// console.log("company", companyData)
				resolve(companyData2.data.contact.name)
			})
			.catch((err) => {
				reject("Error")
			})
...
const getAgent = (requester_id, apikey, subdomain) => {
	return new Promise(function (resolve, reject) {
		const url = `https://${subdomain}.freshdesk.com/api/v2/agents/${requester_id}`
		const options = {
			headers: {
		...
		axios
			...
			.catch((err) => {
				getContact(requester_id, apikey, subdomain, options)
					.then((data) => {
						resolve("Success")
					})
					.catch((error) => {
