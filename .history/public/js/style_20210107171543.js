...
const displayTickets = (companyData, apikey, subdomain, rec) => {
	...
	console.log("rec::", rec)
	console.log(
		"companyData.data.results.length:",
		companyData.data.results.length
	)
	if (rec < companyData.data.results.length) {
		getAgent(companyData.data.results[i].requester_id, apikey, subdomain)
			.then((res) => {
                ...
                <p><b>Ticket ID </b>: ${companyData.data.results[rec].id}&nbsp &nbsp &nbsp<b>Name </b>: &nbsp &nbsp &nbsp<b>Status</b> : ${status_check}&nbsp &nbsp &nbsp<b>Type </b>: ${companyData.data.results[rec].type}</p>
                </div>
                </div>
              </div>`
